...
\end{itemize}
$\mathbb{1}|\psi> = \sum_{j\in\{+\hat{x},-\hat{x}\}}|j\times j||\psi> = <+\hat{x}|\psi>|+\hat{x}>+<-\hat{x}|\psi>|-\hat{x}>=<+\hat{y}|\psi>|+\hat{y}>+<-\hat{y}|\psi>|-\hat{y}>$. $|\psi> \to^{S_x basis} \begin{matrix}<+\hat{x}|\psi>\\<-\hat{x}|\psi>\end{matrix}$, $|\psi> \to^{S_y basis} \begin{matrix}<+\hat{y}|\psi>\\<-\hat{y}|\psi>\end{matrix}$, $|+\hat{z}> \to^{S_x basis} \begin{matrix}<+\hat{x}|+\hat{z}>\\<-\hat{x}|-\hat{z}>\end{matrix} = 1/\sqrt{2}\begin{matrix}1\\1\end{matrix} |+\hat{z}> \to^{S_z basis}\begin{matrix}1\\0\end{matrix}$. $|+\hat{z}> \to^{S_y basis} \begin{matrix}<+\hat{y}|+\hat{z}>\\<-\hat{y}|-\hat{z}>\end{matrix} = 1/\sqrt{2}\begin{matrix}1\\1\end{matrix}$

\marginpar{September 21, 2026}
$\mathbb{1} = P_{+\hat{n}} + P_{-\hat{n}}, \hat{n}\in\{\hat{x},\hat{y},\hat{z}\}$, State : $|\psi> = c_+|+\hat{z}> + C_-|\hat{z}> \to^{S_z basis} \begin{matrix}C_+\\C_-\end{matrix}$. $P_{+\hat{z}}|\psi> = C_+|+\hat{z}> \to^{S_z basis}\begin{matrix}C_+\\0\end{matrix} = C_+\begin{matrix}1\\0\end{matrix}$. Matrix: $P_{+\hat{z}} = \begin{matrix}<+\hat{z}|P_{+\hat{z}}|+\hat{z}>&<+\hat{z}|P_{+\hat{z}}|-\hat{z}>\\<-\hat{z}|P_{+\hat{z}}|+\hat{z}>&<-\hat{z}|P_{+\hat{z}}|-\hat{z}>\end{matrix} = \begin{matrix}1&0\\0&0\end{matrix}$. Check: $\psi = \begin{matrix}C_+\\C_-\end{matrix} : P_{+\hat{z}}\begin{matrix}C_+\\C_-\end{matrix} =\begin{matrix}1&0\\0&0\end{matrix}\begin{matrix}C_+\\C_-\end{matrix} = \begin{matrix}C_+\\0\end{matrix}$. Operator A: $A|\psi> = ?$. n-levels: $\mathbb{1} = \sum_j|j\times j|, <i|j> = \delta_{ij}, A\mathbb{1}|\psi> = \sum_j A|j\times j| |\psi>$. $<k|A|\psi> = \sum_j<k|A|j><j|\psi>(k=1,2,\dots,n)$, $|\psi> \to^{|j> basis}\begin{matrix}<1|\psi>\\<2|\psi>\\<3|\psi>\\\dots\\<n|\psi>\end{matrix} = \psi$, $A\to^{|j> basis}\begin{matrix}A_{11}&A_{12}&\dots&A_{1n}\\A_{21}&\dots&\dots&\dots\\A_{n1}&\dots&\dots&\dots&A_{nn}\end{matrix} = A$, $A_{ij} = <i|A|j>$, $A|\psi> \to A\cdot\psi = \begin{matrix}<1|A|\psi>\\<2|A|\psi>\\<\dots|A|\psi>\\<n|A|\psi>\end{matrix}$.
Rotation Operator
$|\psi'>=R(\theta\hat{y}|\psi>), \theta=\pi/2: \pm\hat{z}\to\pm\hat{x}, |\pm\hat{x}>=R(\pi/2\hat{y})|\pm\hat{z}>, |\psi>=C_+|+\hat{z}>+C_-|-\hat{z}>$, $R(\pi/2\hat{y})|\psi> = C_+R()|+\hat{z}>+C_-R()|-\hat{z}> = C_+|+\hat{x}>+C_-|-\hat{x}>=|\psi'>$, $|\psi'> = R(\theta\hat{y})|\psi>, <\psi'| = <\psi|R(\theta\hat{y})$, $<\psi|R(\theta\hat{y}R(\theta\hat{y}))|\psi>$ for $|+\hat{z}> = |\psi>$, $\theta=\pi/2$, $R(\pi/2)|+\hat{x}> = +\hat{x}>, R^2(\pi/2\hat{y})|+\hat{z}>=R(\pi\hat{y})|+\hat{z}> = |-\hat{z}>$, but $<+\hat{z}|R(\pi\hat{y})|+\hat{z}> = <+\hat{z}|-\hat{z}>=0$ Need $<\psi'|\psi'> = 1$, $|\psi'> = R|\psi> \implies <\psi'|=<\psi|R^+$, $R_{ij} = <i|R|j>, R_{ij}^+ = R_{ji}*$, $\mathbb{1}R^\mathbb{1}=\sum_{ij}|i\times i|R|j\times j| = \sum_{ij}R_{ij}|i\times j|$, $R^+=\sum_{ij}R_{ij}*(|i\times j|)^+ = \sum_{ij}R_{ij}*|j\times i|$. $<\psi'|\psi'>=<\psi|R^+R|\psi>=1$ true for all $R(\theta\hat{n})$ and all $|\psi>$. $\implies R = u$ is unitary: $u^+u=uu^+=\mathbb{1}$.
% ============================================================
\appendix
\section{Appendix}
