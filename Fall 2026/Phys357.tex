...
Rotation Operator
$|\psi'>=R(\theta\hat{y}|\psi>), \theta=\pi/2: \pm\hat{z}\to\pm\hat{x}, |\pm\hat{x}>=R(\pi/2\hat{y})|\pm\hat{z}>, |\psi>=C_+|+\hat{z}>+C_-|-\hat{z}>$, $R(\pi/2\hat{y})|\psi> = C_+R()|+\hat{z}>+C_-R()|-\hat{z}> = C_+|+\hat{x}>+C_-|-\hat{x}>=|\psi'>$, $|\psi'> = R(\theta\hat{y})|\psi>, <\psi'| = <\psi|R(\theta\hat{y})$, $<\psi|R(\theta\hat{y}R(\theta\hat{y}))|\psi>$ for $|+\hat{z}> = |\psi>$, $\theta=\pi/2$, $R(\pi/2)|+\hat{x}> = +\hat{x}>, R^2(\pi/2\hat{y})|+\hat{z}>=R(\pi\hat{y})|+\hat{z}> = |-\hat{z}>$, but $<+\hat{z}|R(\hbar\hat{y})|+\hat{z}> = <+\hat{z}|-\hat{z}>=0$ Need $<\psi'|\psi'> = 1$, $|\psi'> = R|\psi> \implies 
% ============================================================
\appendix
\section{Appendix}
