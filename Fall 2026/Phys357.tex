...
Filter $+\hat{z}$, then filter $+\hat{x}$, then measure $S_z$ again. The beam splits 50/50 into $+\hat{z}$ and $-\hat{z}$, even though every atom was filtered to $+\hat{z}$ at the start. The $S_x$ measurement wiped out the earlier $S_z$ information. Blocking $+\hat{x}$ instead of $-\hat{x}$ gives the same 50/50 result.
...
\begin{definition}[Modified SG$x$ device]
    An SG$x$ device that splits the beam according to $S_x$ and then recombines the two beams, so atoms with $S_x = \pm\hbar/2$ emerge at the same position even though they took two different paths.
\end{definition}

Filter $+\hat{z}$, send it through a modified SG$x$ with neither path blocked, then measure $S_z$. Every atom comes out $+\hat{z}$: 100\%, not the 50/50 of Experiment 3.

\begin{result}
    The modified SG$x$ cannot simply send 50\% of the atoms through in $\ket{+\hat{x}}$ and 50\% in $\ket{-\hat{x}}$: Experiment 3 showed each of those alone gives 50/50 at the end, so that picture is ruled out. Instead the two paths interfere, as for water or light waves, cancelling the possibility of $-\hat{z}$.
\end{result}
...
\begin{remark}[Shot noise]
    ...
    \end{equation}
    This spread is intrinsic, not a flaw of the apparatus: it appears whenever a state with a definite value of one observable (e.g.\ $S_x$ for $\ket{+\hat{x}}$) is measured for an incompatible observable (e.g.\ $S_z$). This is made precise by the uncertainty relations in Chapter 3.
\end{remark}
...
\begin{remark}
    Complex coefficients are required, not just allowed. With real coefficients the relative phases could only be $0$ or $\pi$, so $\gamma - \delta = \pm\pi/2$ would be impossible, and the conditions $|\braket{+\hat{x}|+\hat{z}}|^2 = |\braket{+\hat{y}|+\hat{z}}|^2 = |\braket{+\hat{x}|+\hat{y}}|^2 = \tfrac{1}{2}$ demanded by the Stern-Gerlach experiments could not all hold.
\end{remark}
...
\begin{definition}[Orthonormal and complete basis]
    ...
    \begin{equation}
        \mathbb{1} = \sum_{j=1}^{n}\ket{j}\bra{j}.
    \end{equation}
\end{definition}

\begin{example}
    For $n = 2$, $\mathbb{1} = \ket{+\hat{n}}\bra{+\hat{n}} + \ket{-\hat{n}}\bra{-\hat{n}}$ for any $\hat{n} \in \{\hat{x},\hat{y},\hat{z}\}$, so
    \begin{equation*}
...
% ------------------------------------------------------------
\subsection{Expectation value as an operator}
% ------------------------------------------------------------
...
\begin{remark}
    Write $\ket{\psi} \xrightarrow{S_z} \psi$, not $\ket{\psi} = \psi$. The ket is basis-independent; the column depends on the basis, just like $\vec{E}$ versus its components.
\end{remark}

\begin{example}
    \begin{equation*}
        \ket{\pm\hat{x}} \xrightarrow{S_z} \frac{1}{\sqrt{2}}\begin{pmatrix} 1 \\ \pm 1 \end{pmatrix}, \qquad
        \ket{\pm\hat{y}} \xrightarrow{S_z} \frac{1}{\sqrt{2}}\begin{pmatrix} 1 \\ \pm i \end{pmatrix}.
    \end{equation*}
\end{example}

\begin{example}[Other bases]
    In the $S_x$ basis, $\ket{\psi} \xrightarrow{S_x} \begin{pmatrix} \braket{+\hat{x}|\psi} \\ \braket{-\hat{x}|\psi} \end{pmatrix}$, and similarly for $S_y$. So the same state has different columns:
    \begin{equation*}
        \ket{+\hat{z}} \xrightarrow{S_z} \begin{pmatrix} 1 \\ 0 \end{pmatrix}, \qquad
        \ket{+\hat{z}} \xrightarrow{S_x} \begin{pmatrix} \braket{+\hat{x}|+\hat{z}} \\ \braket{-\hat{x}|+\hat{z}} \end{pmatrix} = \frac{1}{\sqrt{2}}\begin{pmatrix} 1 \\ 1 \end{pmatrix}, \qquad
        \ket{+\hat{z}} \xrightarrow{S_y} \begin{pmatrix} \braket{+\hat{y}|+\hat{z}} \\ \braket{-\hat{y}|+\hat{z}} \end{pmatrix} = \frac{1}{\sqrt{2}}\begin{pmatrix} 1 \\ 1 \end{pmatrix}.
    \end{equation*}
\end{example}
...
\begin{definition}[Projection operator]
    $P_{\pm\hat{n}} = \ket{\pm\hat{n}}\bra{\pm\hat{n}}$, so that $\mathbb{1} = P_{+\hat{n}} + P_{-\hat{n}}$ and $P_{+\hat{z}}\ket{\psi} = c_+\ket{+\hat{z}}$.
\end{definition}
...
\begin{definition}[Matrix of an operator]
    ...
    \begin{equation}
        A_{ij} = \bra{i}A\ket{j}.
    \end{equation}
    Inserting the identity, $\bra{k}A\ket{\psi} = \sum_j\bra{k}A\ket{j}\braket{j|\psi}$, so $A\ket{\psi}$ is represented by the matrix product $A\psi$.
\end{definition}

\begin{example}
    \begin{equation*}
        P_{+\hat{z}} \xrightarrow{S_z} \begin{pmatrix} \bra{+\hat{z}}P_{+\hat{z}}\ket{+\hat{z}} & \bra{+\hat{z}}P_{+\hat{z}}\ket{-\hat{z}} \\ \bra{-\hat{z}}P_{+\hat{z}}\ket{+\hat{z}} & \bra{-\hat{z}}P_{+\hat{z}}\ket{-\hat{z}} \end{pmatrix} = \begin{pmatrix} 1 & 0 \\ 0 & 0 \end{pmatrix},
        \qquad \begin{pmatrix} 1 & 0 \\ 0 & 0 \end{pmatrix}\begin{pmatrix} c_+ \\ c_- \end{pmatrix} = \begin{pmatrix} c_+ \\ 0 \end{pmatrix}.
    \end{equation*}
\end{example}
...
\begin{proposition}[Properties of projectors]
    ...
    \begin{equation}
        P_{-\hat{z}} \xrightarrow{S_z} \begin{pmatrix} 0 & 0 \\ 0 & 1 \end{pmatrix}, \qquad
        \mathbb{1} = P_{+\hat{z}} + P_{-\hat{z}} \xrightarrow{S_z} \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix}.
    \end{equation}
\end{proposition}

\begin{definition}[Adjoint]
    Writing $A = \mathbb{1}A\mathbb{1} = \sum_{i,j}A_{ij}\ket{i}\bra{j}$, the adjoint is
    \begin{equation}
...
\begin{remark}
    The bra of $\ket{\psi'}$ is $\bra{\psi}R^\dagger$, not $\bra{\psi}R$. With the wrong choice, $\ket{\psi} = \ket{+\hat{z}}$ and $\theta = \pi/2$ would give $\braket{\psi'|\psi'} = \bra{+\hat{z}}R(\pi\hat{y})\ket{+\hat{z}} = \braket{+\hat{z}|-\hat{z}} = 0$ instead of 1.
\end{remark}

\begin{theorem}
    Rotation operators are unitary:
    \begin{equation}
        R^\dagger R = RR^\dagger = \mathbb{1}.
    \end{equation}
\end{theorem}
\begin{proof}
    A rotation must preserve normalization: $\braket{\psi'|\psi'} = \bra{\psi}R^\dagger R\ket{\psi} = 1$ for every $\ket{\psi}$ and every $R(\theta\hat{n})$, which forces $R^\dagger R = \mathbb{1}$.
\end{proof}
