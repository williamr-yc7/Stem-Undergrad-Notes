...
\end{remark}
\begin{example}[The exponential as a limit]
    Show that $\lim_{N\to\infty}\left(1+x/N\right)^N = e^x$. Take the logarithm of the left
    side, which turns the power into a product:
    \begin{equation*}
...
\[
    \ket{\psi} = \left(\ket{+\hat{z}}\bra{+\hat{z}} + \ket{-\hat{z}}\bra{-\hat{z}}\right)\ket{\psi}
    = \braket{+\hat{z}|\psi}\ket{+\hat{z}} + \braket{-\hat{z}|\psi}\ket{-\hat{z}},
\]
...
\end{remark}
\begin{example}[A probability by matrix mechanics]
    For $\ket{\psi} = \sqrt{\tfrac{1}{5}}\ket{+\hat{z}} + i\sqrt{\tfrac{4}{5}}\ket{-\hat{z}}$, find $P(S_x = +\hbar/2)$. Write $\ket{\psi}$ as a column and $\ket{+\hat{x}}$ as the conjugate row:
    \[
        \psi \xrightarrow{S_z} \frac{1}{\sqrt{5}}\begin{pmatrix} 1 \\ 2i \end{pmatrix}, \qquad
        \bra{+\hat{x}} \xrightarrow{S_z} \frac{1}{\sqrt{2}}\begin{pmatrix} 1 & 1 \end{pmatrix}.
    \]
    Then
    \[
        \braket{+\hat{x}|\psi} = \frac{1}{\sqrt{10}}\begin{pmatrix} 1 & 1 \end{pmatrix}\begin{pmatrix} 1 \\ 2i \end{pmatrix} = \frac{1+2i}{\sqrt{10}}, \qquad
        P\!\left(S_x = +\tfrac{\hbar}{2}\right) = \frac{|1+2i|^2}{10} = \frac{5}{10} = \frac{1}{2}.
    \]
...
\begin{proof}
    ...
    \[
        \left[(AB)^\dagger\right]_{ij} = (AB)_{ji}^{*} = \sum_k A_{jk}^{*}B_{ki}^{*} = \sum_k (B^\dagger)_{ik}(A^\dagger)_{kj} = \left(B^\dagger A^\dagger\right)_{ij}.
    \]
\end{proof}
\begin{remark}
...
\begin{example}[The matrix of $\hat{S}_z$]
    Here every overlap is $0$ or $1$ by orthonormality, so only the diagonal survives:
    \[
        \bra{+\hat{z}}\hat{S}_z\ket{+\hat{z}} = \frac{\hbar}{2}\left(1 - 0\right) = \frac{\hbar}{2}, \qquad
        \bra{-\hat{z}}\hat{S}_z\ket{-\hat{z}} = \frac{\hbar}{2}\left(0 - 1\right) = -\frac{\hbar}{2},
    \]
...
\end{example}
\begin{example}[The matrix of $\hat{S}_x$]
    ...
    \]
\end{example}
\begin{example}[The matrix of $\hat{S}_y$]
...
\begin{remark}[Shortcut]
    ...
    \[
        \ket{+\hat{x}}\bra{+\hat{x}} \xrightarrow{S_z} \frac{1}{2}\begin{pmatrix} 1 \\ 1 \end{pmatrix}\begin{pmatrix} 1 & 1 \end{pmatrix} = \frac{1}{2}\begin{pmatrix} 1 & 1 \\ 1 & 1 \end{pmatrix}, \qquad
        \ket{-\hat{x}}\bra{-\hat{x}} \xrightarrow{S_z} \frac{1}{2}\begin{pmatrix} 1 & -1 \\ -1 & 1 \end{pmatrix},
    \]
    then subtract and multiply by $\hbar/2$.
\end{remark}
\begin{example}[Expectation value and uncertainty by matrix mechanics]
    For $\ket{\psi} = \sqrt{\tfrac{1}{5}}\ket{+\hat{y}} + i\sqrt{\tfrac{4}{5}}\ket{-\hat{y}}$, find $\braket{S_z}$ and $\Delta S_z$. First move to the $S_z$ basis:
    \[
...
\end{remark}
\begin{example}[$R(\phi\hat{z})$ in the $S_x$ basis]
