...
\begin{definition}[Filter]
    Each device sends atoms into two beams. A filter is a device that blocks on of the beams.
\end{definition}
\begin{remark}
...
Filter the $+\hat{z}$ beam and send it into a second SG$z$. Every atom comes out $+\hat{z}$, none come out $-\hat{z}$. The measurement is repeatable: once an atom is found with $S_z = +\hbar/2$, measuring $S_z$ again gives the same answer. The atom leaves the first device in the state $\ket{+\hat{z}}$. So we end with $N_0/2$ atoms with the $|+\hat{z}>$.

\subsubsection{\texorpdfstring{Experiment 2: SG$z$ then SG$x$}{Experiment 2: SGz then SGx}}

Filter the $+\hat{z}$ beam and send it into an SG$x$. Half the atoms come out $+\hat{x}$ and half come out $-\hat{x}$. Knowing $S_z = +\hbar/2$ tells us nothing about $S_x$. We end up with $N_0/4$ in the $|+\hat{x}>$ and $N_0/4$ in the $|-\hat{x}>$.

\subsubsection{\texorpdfstring{Experiment 3: SG$z$, SG$x$, then SG$z$}{Experiment 3: SGz, SGx, then SGz}}

Filter $+\hat{z}$, then filter $+\hat{x}$, then measure $S_z$ again. The beam splits 50/50 into $+\hat{z}$ and $-\hat{z}$ at the end, even though every atom was filtered to $+\hat{z}$ at the start. The $S_x$ measurement wiped out the earlier $S_z$ information. Blocking $+\hat{x}$ instead of $-\hat{x}$ gives the same 50/50 result. We end with $N_0/8 |+\hat{z}$ and $N_0/8 |-\hat{z}>$.
\begin{result}
...
\begin{definition}[Modified SG device]
    %Too long with useless info be direct what is a modified SG device how is it differenet then a regular one
    An SG device that splits the beam according to $S$ and then recombines the two beams, so atoms with $S = \pm\hbar/2$ emerge at the same position even though they took two different paths.
\end{definition}
Filter $+\hat{z}$, send it through a modified SG$x$ with neither path blocked, then measure $S_z$. Every atom comes out $+\hat{z}$: 100\%, not the 50/50 of Experiment 3. End with $N_0/4 |+\hat{z}>$ from $|+\hat{x}>$ and $N_0/4 |+\hat{z}>$ from $|-\hat{x}>$.
\begin{result}
...
\subsubsection{The state \texorpdfstring{$\ket{+\hat{y}}$}{|+y>}}
\begin{proposition}
    Drop the global phases, $\gamma_+ = \delta_+ = 0$, so that we finally obtain equations for x and y in terms of z:
...
\begin{result}[Phase convention]
    ...
    \begin{equation}
        ...
        \ket{\pm\hat{y}} = \frac{1}{\sqrt{2}}\left(\ket{+\hat{z}} \pm i\ket{-\hat{z}}\right), \qquad
    \end{equation}
    The bras are then
    \begin{equation}
...
\begin{example}
    ...
    \begin{equation*}
        P\!\left(S_y = +\tfrac{\hbar}{2}\right) = |\bra{+\hat{y}}\ket{\psi}|^2 = \frac{1}{2} + \frac{\sqrt{2}}{3}, \qquad
        P\!\left(S_y = -\tfrac{\hbar}{2}\right) = |\bra{-\hat{y}}\ket{\psi}|^2 = \frac{1}{2} - \frac{\sqrt{2}}{3},
    \end{equation*}
...
\begin{definition}[Uncertainty]
    ...
    where 
    \begin{equation}
        \braket{S_z^2} = (\frac{\hbar}{2})^2\left|\braket{+\hat{z}|\psi}\right|^2 + (-\frac{\hbar}{2})^2\left|\braket{-\hat{z}|\psi}\right|^2.
    \end{equation}
\end{definition}
\begin{example}
...
\begin{example}
    ...
    \begin{equation*}
        ...
        \Delta S_z = \frac{\sqrt{2}}{3}\hbar.
    \end{equation*}
\end{example}
\begin{remark}
    $\braket{S_z^2}$ is always equal to $\frac{\hbar^2}{4}$.
\end{remark}
...
\begin{remark}
    The index $j$ labels the outcomes of one chosen observable, not the coordinate axes. Picking $S_x$, $S_y$ or $S_z$ picks the basis; within that basis $j = 1,2$ labels its two states $\ket{a_1} = \ket{+\hat{x}}$ and $\ket{a_2} = \ket{-\hat{x}}$. A spin-1 particle has $n = 3$ outcomes, and an $n$-level atom has $j = 1,\dots,n$.
\end{remark}
...
\begin{proposition}
    Normalization $\braket{\psi|\psi} = \id$ is the statement that the probabilities sum to 1.
\end{proposition}
\begin{proof}
    \begin{equation*}
...
\begin{proposition}[Properties of projectors]
    ...
    \begin{equation}
        ...
        \id = P_{+\hat{z}} + P_{-\hat{z}} \xrightarrow{S_z} \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix}.
    \end{equation}
\end{proposition}
