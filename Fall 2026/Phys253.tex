...
\section{Misc}
Kinetic theory of gases: A microscopic model for U(T) and C
1. Need to determine how P is related to the kinetic energy in the gas. We can generalize from considerations involving on a single gas particle
2. Use the equation of state for an ideal gas to relate this to T. 
momentum is conserved in elastic collisions between gas particles and the walls of container.
Pressure is the force from the particle acted upon a `wall': $F = ma = dp/dt$, where p is the momentum.
Force on the left wall for a gas particle bouncing between the left and right walls at a speed $u_x$, and if you graph force over time, then you see spikes separated by a period of $\Delta T = \frac{2L}{u_x}$. Consider the time interval between collisions with the leftmostwall; $\Delta t = \Delta T = \frac{2L}{U_x}$, $\Delta P_{wall} = -\Delta P_{gas} = -(P_f - P_i)$, $\Delta P_{wall} = -(-mu_x-mu_x) = 2mu_x$, so $2mu_x = \bar{F}\cdot\frac{2L}{u_x}$. $\bar{F} = \frac{mu_x^2}{L}$ for one particle and $\bar{F} = N \cdot \frac{m\bar{u_x^2}}{L}$ for all N particles.
% ============================================================
%  APPENDIX
% ============================================================
\appendix
