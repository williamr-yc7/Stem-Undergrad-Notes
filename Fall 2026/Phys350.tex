...
\begin{proposition}[Potential from the field]
    ...
    The path does not matter since the integral is path-independent. Changing the reference point adds a constant to $V$, which leaves $\vec{E} = -\vec{\nabla}V$ unchanged (gauge freedom). By convention the reference is at infinity, unless the charge distribution extends to infinity.
\end{proposition}
\begin{example}[Point charge]
    Along a radial path,
...
\begin{proposition}[Potential of a distribution]
    ...
    Unlike the integral for $\vec{E}$, there is no $\hat{\mathfrak{r}}$, so computing $V$ first and then $\vec{E} = -\vec{\nabla}V$ is often easier.
\end{proposition}
...
\begin{proposition}[Energy of $N$ point charges]
    The first charge comes in free, and each later one feels the potential of those already placed, giving one term per pair:
    \[U = \frac{1}{2}\sum_{i\neq j}\frac{q_iq_j}{4\pi\epsilon_0|\vec{r}_i - \vec{r}_j|},\]
    where the $\tfrac12$ corrects for counting each pair twice.
\end{proposition}
\begin{remark}
    $U$ does not obey superposition: it is quadratic in the charges, so the energy of two configurations combined includes cross terms. %How? since U is a sum
\end{remark}
...
\end{theorem}
\begin{proof}
    By Poisson's equation, $U = -\frac{\epsilon_0}{2}\int(\nabla^2 V)V\,d\tau$. In index notation, $\partial_i(V\partial_i V) = (\partial_i V)(\partial_i V) + V\partial_i\partial_i V$, so
    \[(\nabla^2 V)V = \vec{\nabla}\cdot(V\vec{\nabla}V) - |\vec{\nabla}V|^2.\]
    Then
    \[U = \frac{\epsilon_0}{2}\int|\vec{\nabla}V|^2\,d\tau - \frac{\epsilon_0}{2}\oint_{\infty}V\vec{\nabla}V\cdot d\vec{a}.\]
    For a bounded distribution $V \sim 1/r$ and $|\vec{\nabla}V| \sim 1/r^2$ while the area grows as $r^2$, so the surface term falls off as $1/r$ and vanishes. With $\vec{E} = -\vec{\nabla}V$, $U = \frac{\epsilon_0}{2}\int E^2\,d\tau$.
\end{proof}
