...
\subsection{Proving the Identities}%read
\subsubsection{Curl of a Curl}
Set $\vec{G} \equiv \vec{\nabla}\times(\vec{\nabla}\times\vec{F})$. Using $\varepsilon_{ijk} = \varepsilon_{kij}$ to bring the summed index forward, then the epsilon-delta identity,
\begin{align}
    G_i &= \sum_{j,k}\varepsilon_{ijk}\partial_j\sum_{m,n}\varepsilon_{kmn}\partial_m F_n
    = \sum_{j,m,n}\left(\delta_{im}\delta_{jn} - \delta_{in}\delta_{jm}\right)\partial_j\partial_m F_n.
\end{align}
The first term is $\sum_j \partial_i\partial_j F_j = \left(\vec{\nabla}(\vec{\nabla}\cdot\vec{F})\right)_i$ and the second is $-\sum_j\partial_j\partial_j F_i = -(\nabla^2\vec{F})_i$, giving $\vec{\nabla}\times(\vec{\nabla}\times\vec{F}) = \vec{\nabla}(\vec{\nabla}\cdot\vec{F}) - \nabla^2\vec{F}$.

\subsubsection{Curl of a Gradient}
Since $j, k$ are summed, relabelling $j \leftrightarrow k$ and using the antisymmetry of $\varepsilon$ and commuting partials,
\[
    \left(\vec{\nabla}\times(\vec{\nabla}f)\right)_i = \sum_{j,k}\varepsilon_{ijk}\partial_j\partial_k f
    = \sum_{j,k}\varepsilon_{ikj}\partial_k\partial_j f = -\sum_{j,k}\varepsilon_{ijk}\partial_j\partial_k f.
\]
The expression equals minus itself, so it is zero: $\vec{\nabla}\times(\vec{\nabla}f) = 0$.
