...
\begin{definition}[Matrix Solution]
    ...
    \begin{equation}
        ...
        y' = A(t)y
    \end{equation}
    is an $n \times n$ matrix function whose columns are solutions to \{\textup{\ref{eq:ode-system-matrix}}\}.
\end{definition}
...
\begin{proposition}
    Let $y(t)$ be a matrix solution of \{\textup{\ref{eq:ode-system-matrix}}\}. Either
...
\begin{definition}
A matrix solution with nonvanishing Wronskian is called a fundamental matrix solution.
\end{definition}

\begin{theorem}[Liouville's formula]
Let $y(t)$ be a matrix solution. Then
\[
\det(y(t))
=
\det(y(t_0))
e^{\int_{t_0}^t \operatorname{tr}(A(s))\,ds}.
\]
Here,
\[
\operatorname{tr}(M)
=
\text{sum of the diagonal entries of } M \in M_n(\mathbb{R}).
\]
\end{theorem}
...
y' = A(t)y,
\]
the goal is to find $n$ linearly independent solutions
\[
y_1, y_2, \ldots, y_n
\]
such that $W(t_0) \neq 0$.
...
y(t)
=
c_1 y_1(t) + c_2 y_2(t) + \cdots + c_n y_n(t),
\qquad
