...
\end{problem}
\begin{problem}[Existence and Uniqueness Theorem 5]
    Consider the initial value problem
...
\subsection{First order linear equations}%to organize
...
\begin{example}
    ...
    \[e^{-2t}(y' - 2y) = \frac{d}{dt}(e^{-2t}y) = 3e^{-t}.\] 
...
\begin{example}[Falling Object with Air Resistance]
    \[v' + \frac{\gamma}{m}v = g\]
    Integrating factor: \(\mu(t) = e^{\frac{\gamma}{m}t}\)
    \[\frac{d}{dt}(\mu v) = g\mu \implies v = \frac{gm}{\gamma} + Ce^{-\frac{\gamma}{m}t}\]
    With \(v(0) = \frac{gm}{\gamma}\), \(C = 0\), so \(v(t) = \frac{gm}{\gamma}\)
\end{example}

\begin{example}[Mixing Problem]
    Tank contains 100 L brine with \(y_0\) g salt. Inflow: 50 g/L at rate \(R\), outflow: \(R\) L/s.  
    \[y' = 50R - \frac{R}{100}y \implies y' + \frac{R}{100}y = 50R, \quad y(0) = y_0\]
    Integrating factor: \(\mu(t) = e^{\frac{R}{100}t}\)
    \[y(t) = 5000 + (y_0 - 5000)e^{-\frac{R}{100}t}\]
    Threshold 5000: exponential approaches from above and below.
\end{example}
...
\end{solution}
\begin{solution}[Existence and Uniqueness Theorem 5]
    On
