...
Let $y'(t) = f(y(t))$, $f$ is LLC, and $y_0 \in \mathbb{R}^n$. Denote by
\[\Gamma_{y_0} = \{y(t) \mid y'(t) = f(y(t)), y(0) = y_0, t \in J_{max}\},\]
where $J_{max}$ is the maximal time interval on which the solution of the IVP $y'(t) = f(y(t)), y(0) = y_0$ is defined. The set $\Gamma_{y_0}$ is called the orbit of $y_0$, and it is a curve in $\mathbb{R}^n$.
\begin{definition}[Phase Portrait]
    The phase portrait is the set of all orbits.
\end{definition}
\begin{example}
    Let $y' = y(1-y)$, since this is a separable ODE, we have
    \[y(t) = \frac{y_0}{y_0 + (1-y_0)e^{-t}}.\]
    The phase portrait is the set of all orbits. The orbit of 0 is $\Gamma_0 = \{0\}$. The orbit of 1 is $\Gamma_1 = \{1\}$. For $y_0 \in (0,1)$, $\Gamma_{y_0} = (0,1)$. For $y_0 > 1$, $\Gamma_{y_0} = (1,+\infty)$. For $y_0 < 0$, $\Gamma_{y_0} = (-\infty,0)$. $0,1$ are the equilibrium solutions.
\end{example}
\begin{definition}
    A vector $\tilde{y} \in \mathbb{R}^n$ is called an equilibrium solution (steady state) of the ODE $y' = f(y)$, $f(\tilde{y})=0\in\mathbb{R}^n$.
\end{definition}
\begin{example}
    Let $y'=Ay=f(y)$. The equilibrium solutions are given by $\ker(A) = \{u\mid Au=0\}$. If A is invertible $Ay = 0 \implies y = A^{-1}Ay = 0$. The $0$ is the only equilibrium solution. If A is non-invertible, then $\ker(A)$ contains more.
\end{example}
\begin{example}
    Let $A = \begin{pmatrix}1&-1\\2&-2\end{pmatrix}$, then $Au = 0$:
    \[\begin{pmatrix}1&-1\\2&-2\end{pmatrix}\begin{pmatrix}a\\b\end{pmatrix} = \begin{pmatrix}0\\0\end{pmatrix}.\]
    Solving we get $a=b$, which implies $\begin{pmatrix}1\\1\end{pmatrix}\subseteq \ker(A)$.
    \[\ker(A) = <\begin{pmatrix}1\\1\end{pmatrix}> = \left\{c\begin{pmatrix}1\\1\end{pmatrix} \mid c\in\mathbb{R}\right\}\]
\end{example}
\begin{example}
    Using Newton's 2nd law and trigonometric identities:
    \[\theta'' + \gamma \theta' + \frac{g}{l}sin(\theta) = 0.\]
    Let $y_1=\theta$,$y_2=\theta'$, we have:
    \[\begin{pmatrix}y_1'\\y_2'\end{pmatrix}=\begin{pmatrix}y_2\\-\frac{g}{l}sin(y_1)-\gamma y_2\end{pmatrix}=f(y).\]
    An equilibrium solution which satisfy $f(y) = 0$ is $y_2=0$ and $sin(y_1)=0$, which implies $y_1=n\pi, n\in\mathbb{Z}$.
\end{example}
\begin{definition}
    An equilibrium solution $\tilde{y}$ of $y'=f(y)$ is stable if $\forall \epsilon >0$, tere exists $\delta = \delta(\epsilon)$ such that for every solution of $y'=f(y)$ with $||y(0) - \tilde{y}|| \leq \delta \implies ||y(t) - \tilde{y}|| \leq \epsilon, \forall t\geq0$. In other words, if one starts close to $\tilde{y}$, then the solution remains close to $\tilde{y}$. 
    
    $\tilde{y}$ satisfies $f(\tilde{y})=0$, $\Gamma_{\tilde{y}}=\{\tilde{y}\}$
    \[(\tilde{y})' = 0 = f(\tilde{y}) \implies \tilde{y}(t) = \tilde{y}, \forall t\in\mathbb{R}\]
\end{definition}
\begin{example}
    Let $y' = -2y$, then $\tilde{y} = 0$, $y(t) = e^{-2t}y(0)$.
    \[|y(t) - 0| = e^{-2t}|y(0)-0| \leq |y(0)-0| \text{for }t\geq0\]
    Let $\epsilon > 0$, and $\delta = \epsilon$. Indeed, if $|y(0)| \leq \delta = \epsilon \implies |y(t) - 0| \leq \epsilon, \forall t \geq 0$.
\end{example}

$\tilde{y}$ is unstable if $\exists \epsilon > 0$ such that for every $\delta >0$ (small enough), then there exists $T>0$, such that there is a solution $y(t)$ with $||y(0)-\tilde{y}|| \leq \delta$ and $||(y(T) - \tilde{y})|| > \epsilon$.

$\tilde{y}$ is asymptotically stable if it is stable and there $\alpha > 0$ such that if $||y(0)-\tilde{y}|| \leq \alpha$, then $\lim_{t\to +\infty}||y(t)-\tilde{y}||=0$.

\marginpar{March 12, 2026}
\begin{example}
    Let $y' = y(1-y)$, and $r=k=1$ (logistic equation). Let us show (using the definition) that $\tilde{y} = 0$ is an unstable equilibrium solution. Let $\epsilon = \frac{1}{2}$, pick $\delta >0$, and smaller than 1. Choose the initial condition to be $y(0) = \delta$. The solution of the IVP is given by
    \[y(t) = \frac{\delta}{\delta + (1-\delta)e^{-t}}.\]
    Clearly, $|y(0)-0| = \delta$. There exists $T = T(\delta)$ such that $y(T) \geq \frac{1}{2}$, that is $|y(T) - 0| \geq \epsilon = \frac{1}{2} \implies 0$ is unstable. Let us now show that $\tilde{y} = 1$ is an asymptotically stable equilibrium solution. Let $\epsilon > 0$. For any $y_0 > 0$, the solution is
    \[y(t) = \frac{y_0}{y_0 + (1-y_0)e^{-t}}.\]
    \[y(t) - \tilde{y}| = |\frac{y_0}{y_0 + (1-y_0)e^{-t}} - 1| = |\frac{(y_0-1)e^{-t}}{y_0 + (1-y_0)e^{-t}}| = |\frac{y_0 -1}{y_0e^t + (1-y_0)}| \leq |y_0 - 1|\]
    But $y_0 e^t + (1-y_0) = y_0(e^t-1) + 1 \geq 1$ only since $e^t - 1 \geq 0$ for $t \geq 0$. For every $t \geq 0$, $|y(t) - \tilde{y}| \leq |y(0) - \tilde{y}| \leq \delta = \epsilon$, this implies that $\tilde{y} = 1$ is stable.
\end{example}
\begin{example}
    Let $y' = Ay$, and
    \[A = \begin{pmatrix}-3/2&1/2\\1/2&-3/2\end{pmatrix}.\]
    Since $det(A) \neq 0 \implies \ker(A) = \left\{\begin{pmatrix}0\\0\end{pmatrix}\right\} \implies \tilde{y} = \begin{pmatrix}0\\0\end{pmatrix}$ is the only equilibrium solution. Let us compute $e^{At} \implies y(t) = e^{At}y_0$. Compute $y(t) = \begin{pmatrix}-e^{-2t}&e^{-t}\\e^{-2t}&e^{-t}\end{pmatrix} \implies e^{At} = y(t)y^{-1}(0) = \frac{1}{2}\begin{pmatrix}e^{-2t}+e^{-t}&-e^{-2t}+e^{-t}\\-e^{-2t}+e^{-t}&e^{-2t}+e^{-t}\end{pmatrix}$. The unique solution to the IVP is $y(t) = e^{At}y_0$. 
    
    Let us show that 0 is asymptotically stable. Let $\epsilon > 0$. Choose the sup norm on $\mathbb{R}^2$.
    \[||x||_\infty = max\{|x_1|,|x_2|\}\]
    \[||y(t) - 0||_\infty = ||e^{At}y(0)||_\infty\]
    Recall that given $M \in M_n(\mathbb{R})$, the induced sup matrix norm
    \[||M||_\infty = \max_{||x||_\infty = 1}||Mx||_\infty = \max_{1\leq i \leq n}\sum_{j=1}^{n}|M_{i,j}|\]
    this is the max of the sum of the absolute value of each row.
    \[\implies ||Mx||_\infty \leq ||M||_\infty \cdot ||x||_\infty\]
    Here,
    \[||e^{At}||_\infty = e^{-t}\max\left\{\frac{1}{2}|e^{-t}+1|+\frac{1}{2}|e^{-t}+1|+\frac{1}{2}|e^{-t}+1|+\frac{1}{2}|e^{-t}+1|\right\} \leq 2e^{-t}\]
\end{example}
\begin{example}
    Let $y' = Ay$, and
    \[A = \begin{pmatrix}-3/2&1/2\\1/2&-3/2\end{pmatrix}.\]
    \[\implies ||y(t)||_\infty \leq 2e^{-t}||y(0)|| \leq 2e^{-t}\delta = e^{-t}\epsilon \leq \epsilon, \forall t\geq 0\]
    Pick $\delta = \delta(\epsilon) = \frac{\epsilon}{2}$, this implies $\begin{pmatrix}0\\0\end{pmatrix}$ is stable. Pick any $\alpha>0$. If $||y(0)||_\infty \leq \alpha$, then $||y(t)||_\infty \leq 2e^{-t} ||y(0)||_\infty = 2\alpha e^{-t} \implies \lim_{t \to +\infty}y(t) = \begin{pmatrix}0\\0\end{pmatrix} \implies 0$ is asymptotically stable.
\end{example}

\subsection{Phase Portaits of Planar Linear Systems}
Let \eqref{eq:ode-system-matrix}, $A \in M_2(\mathbb{R})$, and assume $det(A) \neq 0$.
\begin{enumerate}
    \item A is diagonalizable with real eigenvalues and the general solution is
    \[y(t) = c_1e^{\lambda_1t}u_1+c_2e^{\lambda_2t}u_2\]
\end{enumerate}
