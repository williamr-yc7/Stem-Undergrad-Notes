...
\begin{solution}[Complex eigenvalues and real-valued solutions]
    a.
    \[p(\lambda) = \det(A - \lambda I) = \det\begin{vmatrix}-3-\lambda & -2 \\ 5 & 3-\lambda\end{vmatrix} = \lambda^2 + 1\]
    Set the determinant to be $0$,
    \[\lambda^2 + 1 = 0\]
    Solving, gives us two eigenvalues $\lambda_1 = i$ and $\lambda_2 = -i$. Now we need to find some eigenvectors.
    For $\lambda_1 = i$,
    \[\det\begin{pmatrix}-3-i & -2 \\ 5 & 3-i\end{pmatrix}\begin{pmatrix}a\\b\end{pmatrix} = 0\]
    Solving for $a$ and $b$, we find that $a = \frac{2}{-3-i}b$, giving us the first eigenvector $u_1 = \begin{pmatrix}\frac{2}{-3-i}\\1\end{pmatrix}$.

    For $\lambda_2 = -i$,
    \[\det\begin{pmatrix}-3+i & -2 \\ 5 & 3+i\end{pmatrix}\begin{pmatrix}a\\b\end{pmatrix} = 0\]
    Solving for $a$ and $b$, we find that $a = \frac{2}{-3+i}b$, giving us the first eigenvector $u_1 = \begin{pmatrix}\frac{2}{-3+i}\\1\end{pmatrix}$.
    Answer:
    \[\lambda_1 = i, \vec{v_1} = \begin{pmatrix}\frac{2}{-3-i}\\1\end{pmatrix}, \quad \lambda_2 = -i, \vec{v_2} = \begin{pmatrix}\frac{2}{-3+i}\\1\end{pmatrix}\]
