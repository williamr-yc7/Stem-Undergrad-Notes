...
If $\mu = \mu(x)$, then $\mu_y = 0 \implies \mu\frac{(M_y - N_x)}{N} = \mu_x = \frac{d\mu}{dx}$. Hence, IF the function $\frac{(M_y - N_x)}{N}$ only depends on x, we can solve the separable ODE $\mu\frac{(M_y - N_x)}{N} = \mu_x = \frac{d\mu}{dx}$ and we find the integrating factor $\mu(x)$, we use it to transform the original into an exact ODE, which we then solve.
\begin{example}
