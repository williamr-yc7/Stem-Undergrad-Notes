...
\end{example}
\begin{example}%to read
    Let
    \[A = \begin{pmatrix}1&1\\0&1\end{pmatrix}\]
    \[p(\lambda) = \begin{vmatrix}1-\lambda&1\\0&1-\lambda\end{vmatrix} = (\lambda-1)^2-1 = 0\]
    \[\implies \lambda_1 = \lambda_2 = 1\]
    Find the eigenvectors:
    \[(A-I)u_1 = 0\]
    \[\implies \begin{pmatrix}0&1\\0&0\end{pmatrix}\begin{pmatrix}a\\b\end{pmatrix} = \begin{pmatrix}0\\0\end{pmatrix} \implies b = 0.\]
    Pick any values of $a$, say $a=1$, then there is only one eigenvector which is
    \[u = \begin{pmatrix}1\\0\end{pmatrix}\]
\end{example}
\begin{theorem}
    Let $A \in M_n(\mathbb{R})$ be diagonalizable. Let $\sigma(A) = \{\lambda_1,\ldots,\lambda_n\}$ (potentially complex valued). Since A is diagonalizable, denote by $u_1,\ldots,u_n$ the associated eigenvectors (potentially complex). The general solution (potentially complex) is of \textup{\eqref{eq:ode-system-matrix}}
    \[y(t) = \sum_{i=1}^n c_ie^{\lambda_i t}u_i.\]
\end{theorem}
Question: What if some eigenvalues are complex?

\subsubsection{Complex eigenvalues and real-valued solutions}
If $A$ has real entries and a complex eigenpair $(\lambda,u)$ with $\lambda=\alpha+i\beta$ and $u=p+iq$ (where $p,q\in\mathbb{R}^n$), then a complex solution is
\[y(t)=e^{\lambda t}u=e^{(\alpha+i\beta)t}(p+iq).\]
Using $e^{i\beta t}=\cos(\beta t)+i\sin(\beta t)$ and taking real and imaginary parts gives two
real solutions:
\[y^{(1)}(t)=e^{\alpha t}\big(p\cos(\beta t)-q\sin(\beta t)\big), \qquad y^{(2)}(t)=e^{\alpha t}\big(p\sin(\beta t)+q\cos(\beta t)\big).\]
...
\begin{example}
    ...
    Giving us two possible eigenvalues $\lambda_1 = 2 + i$ and $\lambda_2 = 2 - i$. Find the eigenvectors:
    \[(A-2I)u_1 = 0\]
    \[\implies \begin{pmatrix}i&-1\\1&i\end{pmatrix}\begin{pmatrix}a\\b\end{pmatrix} = \begin{pmatrix}0\\0\end{pmatrix} \implies b = ia.\]
    Pick any values of $a$, say $a=1$, then the first eigenvector is
    \[u_1 = \begin{pmatrix}1\\i\end{pmatrix}\]
    
    Let $\overline{z}$ denote the complex conjugate of $z \in \mathbb{C}$. Similarly, for $u \in \mathbb{C}^n$, $\overline{u}$ is the component-wise complex conjugate.
    \[\overline{Au_1} = \overline{\lambda_1u_1}\]
    \[\implies A\overline{u_1} = \overline{\lambda_1u_1}\]
    since $\overline{A} = A$ cuz A is real.
    
    Conclusion, if $A \in M_n(\mathbb{R})$ and if $(\lambda,u)$ is an eigen pair, then $(\overline{\lambda},\overline{u})$ is also an eigenpair.
\end{example}
Assume that $(\lambda,u)$ is a complex eigenpair, then $y(t) = e^{\lambda t}u$ is a complex-valued solution to \eqref{eq:ode-system-matrix}. Denote $\lambda = \alpha + i\beta$ (eigenvalue) and $u + iv$ (eigenvalue).
\begin{definition}[Euler's Formula]
    Let $x \in \mathbb{R}$. Then the Euler's formula is
    \[e^{ix} = \cos(x) + i\sin(x).\]
    We can transform it into a series as
    \[e^z = \sum_{n=0}^{\inf}\frac{1}{n!}z^n.\]
\end{definition}
\[y(t) = e^{(\alpha + i\beta)t}(u+iv) = e^{\alpha t}e^{i\beta t}(u+iv) = e^{\alpha t}(cos(\beta t)+i\sin(\beta t))(u+iv) =  e^{\alpha t}(cos(\beta t)u - \sin(\beta t)v) + ie^{\alpha t}(\sin(\beta t)u + \cos(\beta t)v).\]
where $y_1(t) = e^{\alpha t}(cos(\beta t)u - \sin(\beta t)v)$ and $y_2(t) = e^{\alpha t}(\sin(\beta t)u + \cos(\beta t)v)$.
Exercise: Verify that $y_1(t)$, $y_2(t)$ are real valued solutions to \eqref{eq:ode-system-matrix}.
...
\vec y(t) = e^{it} u_1.
\]
Using Euler's formula $e^{it}=\cos t + i\sin t$, we write
\[
\vec y(t)
= (\cos t + i\sin t)
\begin{pmatrix}
...
\boxed{
\begin{aligned}
y_1(t) &= 7\cos t + 9\sin t,\\
y_2(t) &= -15\cos t - 10\sin t.
\end{aligned}
}
\]
\end{solution}
