...
\begin{solution}[Complex eigenvalues and real-valued solutions]
    ...
    From the eigenpair $(\lambda_1,u_1)$, a complex-valued solution is
    \[\vec y(t) = e^{it} u_1.\]
    Using Euler's formula \eqref{eq:EulerFormula}, we write
    \[\vec y(t) = (\cos t + i\sin t)\begin{pmatrix}\dfrac{2}{-3-i}\\1\end{pmatrix}.\]
    Separating real and imaginary parts yields two real-valued solutions:
    \[\vec y^{(1)}(t) = \begin{pmatrix}2\cos t \\ -3\cos t + \sin t\end{pmatrix}, \qquad \vec y^{(2)}(t) = \begin{pmatrix} 2\sin t \\ -3\sin t - \cos t \end{pmatrix}.\]
    Therefore, the general real solution is
    \[\vec y(t) = c_1 \vec y^{(1)}(t) + c_2 \vec y^{(2)}(t).\]
    Applying the initial conditions $y_1(0)=7$ and $y_2(0)=-15$, we compute
    \[\vec y^{(1)}(0)=\begin{pmatrix}2\\-3\end{pmatrix}, \qquad \vec y^{(2)}(0)=\begin{pmatrix}0\\-1\end{pmatrix}.\]
    Thus,
    \[c_1\begin{pmatrix}2\\-3\end{pmatrix} + c_2\begin{pmatrix}0\\-1\end{pmatrix} = \begin{pmatrix}7\\-15\end{pmatrix}.\]
    Solving gives
    \[c_1 = \frac{7}{2}, \qquad c_2 = \frac{9}{2}.\]
    Hence, the real-valued solution of the initial value problem is
    \[\boxed{\begin{aligned}y_1(t) &= 7\cos t + 9\sin t,\\y_2(t) &= -15\cos t - 10\sin t.\end{aligned}}\]
\end{solution}
