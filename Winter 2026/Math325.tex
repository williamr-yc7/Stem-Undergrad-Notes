...
\begin{definition}[Matrix Exponential]
    For $A \in M_n(\mathbb{R})$, the exponential of $A$ is defined by the power series
    \[e^A = \sum_{k=0}^{\infty} \frac{A^k}{k!} = I + A + \frac{A^2}{2!} + \frac{A^3}{3!} + \cdots .\footnote{This definition is motivated by the scalar identity $e^x = \sum_{k=0}^{\infty} \frac{x^k}{k!}$.
\]}
\end{definition}

\begin{definition}[Induced Matrix Norm]
    Given any norm $||\cdot||$ on $\mathbb{R}^n$, the induced matrix norm is defined by 
    \[||A|| = \sup_{x \neq 0} \frac{||Ax||}{||x||} = \sup_{||x|| = 1} ||Ax|| < \infty.\]

    Properties:
    \[||Ax|| \leq ||A|| \cdot ||x||, \quad \forall x \in \mathbb{R}^n.\]
\end{definition}
By definition, if we take two matrices $A,B \in M_n(\mathbb{R})$,
