...
\begin{definition}[Potential Function]
    ...
    \begin{equation}
        \Psi(x,y) = H_1(x) + H_2(y),
    \end{equation}
    where $H_1$ and $H_2$ are antiderivatives of $M(x)$ and $N(y)$ from \{\textup{\ref{SeparableODE}}\}.
\end{definition}
...
\begin{definition}[Exact Equation]
    The first-order ODE \{\textup{\ref{SeparableODE}}\} is called \emph{exact} on a domain $D$ if there exists a function $\Psi(x,y)$ (a potential function) such that
...
\begin{theorem}
    ...
    Then the first-order equation \{\textup{\ref{SeparableODE}}\} is exact on $D$ if and only if $M_y(x,y) = N_x(x,y)$ for all $(x,y)\in D$.
\end{theorem}
\begin{proof}
...
\begin{equation}
    \label{eq:ode-system}
    y'(t) = A(t)y(t) + r(t), \quad t \in I,
\end{equation}
where $y(t)$ is the unknown vector, $A(t)$ is a matrix of coefficients, and $r(t)$ is a forcing term. Explicitly,
...
\subsection{Theory of first order linear systems}%to read

\begin{theorem}[Existence and uniqueness]
If every entry of \(A(t)\) and \(r(t)\) is continuous on an interval \(I\), then for any
\[
t_0 \in I, \quad y_0 \in \mathbb{R}^n,
\]
the IVP
\[
\begin{cases}
...
\end{cases}
\]
has a unique solution on all of \(I\). Hence \(J_{\max}=I\).
\end{theorem}

\begin{definition}
If \(r(t)=0\), the system is \textbf{homogeneous}.  
Otherwise it is \textbf{non-homogeneous}.
\end{definition}

We first study the homogeneous system
\[
y'(t)=A(t)y(t). \tag{4}
\]
...
Let
\[
S=\{\, y:I\to\mathbb{R}^n \mid y \text{ solves (4)} \,\} \subset C^1(I).
\]
...
\begin{definition}
Functions \(y_1,\dots,y_n : I \to \mathbb{R}^n\) are \textbf{linearly independent} if
\[
c_1y_1(t)+\cdots+c_ny_n(t)=0 \ \text{for all } t
...
\begin{theorem}
The solution set \(S\) is an \(n\)-dimensional vector subspace of \(C^1(I)\).
\end{theorem}

\begin{proof}
\textbf{Step 1: Vector space.}

If \(y_1,y_2 \in S\) and \(\alpha_1,\alpha_2 \in \mathbb{R}\),
\[
(\alpha_1 y_1 + \alpha_2 y_2)'
=
\alpha_1 y_1' + \alpha_2 y_2'
=
A(t)(\alpha_1 y_1 + \alpha_2 y_2),
\]
so \(\alpha_1 y_1 + \alpha_2 y_2 \in S\). Also \(0 \in S\).

\textbf{Step 2: Construct a basis.}

Let \(e_1,\dots,e_n\) be the standard basis of \(\mathbb{R}^n\).

For each \(i\), let \(y_i\) be the unique solution of
\[
\begin{cases}
y' = A(t)y, \\
y(t_0) = e_i.
...
Every linear combination of the \(y_i\) solves the system, so
\(\operatorname{span}(B)\subset S\).
...
\[
z(t)=\sum_{i=1}^n z_i\,y_i(t),
\]
so \(S\subset \operatorname{span}(B)\).
...
\]
hence \(c_1=\cdots=c_n=0\).

Thus \(B\) is a basis and \(\dim S=n\).

BTW $S = \{y : I \to \mathbb{R}^n |\text{ y is a solution}\}$.
\end{proof}

\marginpar{February 5, 2026}
Definition: A matrix solution $y(t)$ of equation $y' = A(t)y$ (*1) is a $n \times n$ matrix function whose columns are solutions to (*1).
By the POS (princicple of superposition), for any fixed vector $C = \begin{pmatrix}C_1\\C_2\\\vdots\\C_n\end{pmatrix} \in \mathbb{R}^n$.
\[y(t)c = c_1y_1(t) + c_2y_2(t) + \cdots + c_ny_n(t) \in S\]
Proposition: Let $y(t)$ be a matrix solution of  (*1). Either $det(y(t)) \neq 0$ $\forall t \in I$ or $det(y(t)) = 0$.
Proof: Assume that there exist $t_0 \in I$ such that $det(y(t_0)) = 0 \implies y(t_0)$ is not invertible.
$\implies \exists$ a non zero vector $v \in \mathbb{R}^n$ such that $y(t_0)v = 0 = \begin{pmatrix}0\\0\\\vdots\\0\end{pmatrix} \in \mathbb{R}^n$. Let $y_1(t) = y(t)v \in S$ which satisfies the initial condition $y_1(t_0) = y(t_0)v = 0$. But $y_2(t) = 0 \in S$.
By the theorem of existence and uniqueness, $y_1(t) = y(t)v = y_2(t) = 0 \implies det(y(t)) = 0 \forall t \in I$.

Definition:$w(t) = w(y_1,\ldots,y_n)(t) = det(y(t))$ is called the Wronskian of $y_1,\ldots,y_n$.

Definition: A matrix solution with non banishing wronskian is called a fundamental matrix solution.

Theorem (Louville's formula): Let $y(t)$ a matrix solution. Then $det(y(t)) = det(y(t_0))e^{\int_{t_0}^t tr(A(s))ds}$. Here, $tr(M) =$ Sum of the diagonal entries of $M \in M_n(\mathbb{R})$.

$y' = A(t)y$. Goal: Find n linearly independent solutions $y_1,y_2,\ldots,y_n$ show that $W(t_0) \neq 0$.
$\implies y(t) = c_1y_1(t) + c_2y_2(t) + \cdots + c_ny_n(t)$, for $c_1,\ldots,c_n \in \mathbb{R}$ (GENERAL SOLUTION).

Given an initial condition $y(t_0) = y_0 \in \mathbb{R}^n$.
\[\implies y(t_0) = c_1y_1(t_0) + \ldots + c_ny_n(t_0) = \begin{pmatrix}
    |&|&|\\y_1(t_0)&y_2(t_0)&y_n(t_0)\\|&|&|
...
$\implies$The unique $(c_1,\ldots,c_n)$ is given by
...
\[y' = Ay \text{(*2)}, \qquad A \in M_n(\mathbb{R})\text{ with constant entries}\]
We look for solutions of the form $y(t) = e^{\lambda t}u$ (*3), $U$ is a constant vector.
\[a_0y^n + a_1y^{n+1} + \cdots + a_{n-1}y' + a_ny = 0\]
Look for solutions of the form $e^{rt}$

Plug (*3) in (*2):
\[y'(t) = \frac{d}{dt}(e^{\lambda t}u) = \lambda e^{\lambda t}u = e^{\lambda t}(\lambda u) = Ay = A(e^{\lambda t}u) = e^{\lambda t}(Au) \implies Au = \lambda u \text{(*4)}\]
In other words, $\lambda$ is an eigenvalue of A and u is a corresponding eigenvector.

Definition: Given a matrix $A \in M_n(\mathbb{C})$, denote by $ker A = \{u | Au = 0\}$, it is called the kernel (or null space) of A.

Equation (*4) $\iff Au - \lambda u = 0 \iff Au - \lambda Iu = 0$, where I is the identity matrix $\in M_n(\mathbb{R})$, $\iff (A - \lambda I)u = 0$.

Define $p(\lambda) = det(A - \lambda I)$. It is called the characteristic polynomial of A. An eigenvalue is a root of $p(\lambda)$ that is, it solves $p(\lambda) = det(A - \lambda I) = 0$.

Definition: Given a matrix A, we denote by $\sigma(A) = \{\lambda | p(\lambda) = 0\}$ the set of eigenvalues of A. It is called the spectrum of A.

Example:
\[y' = Ay\]
where $y = \begin{pmatrix}y_1\\y_2\end{pmatrix}$ and $A = \begin{pmatrix}1&3\\3&1\end{pmatrix}$.
\[p(\lambda) = det(A - \lambda I) = \begin{pmatrix}1 - \lambda & 3\\3 & 1 - \lambda\end{pmatrix} = (1 - \lambda)^2 - 9 = 0\]
\[\iff (1 - \lambda)^2 - 9 = 0 \iff 1 - \lambda = \pm 3\]
$\implies \lambda = \lambda_1 = -2, \lambda = \lambda_2 = 4$ are the two eigenvalues $\implies \sigma(A) = \{-2,4\}$.

Eigenvectors: for $u_1$ : $(A - \lambda I)u_1 = 0$
\[\begin{pmatrix}3&3\\3&3\end{pmatrix}\begin{pmatrix}a\\b\end{pmatrix} = \begin{pmatrix}0\\0\end{pmatrix}\]
\[\therefore b = -a\]
You can pick any values of a and b would be the negative of a, lets pick $a = 1$..

for $u_2$ : $u_2 = \begin{pmatrix}1\\1\end{pmatrix} \implies y_1(t) = e^{-2t}\begin{pmatrix}1\\-1\end{pmatrix} \in S$ and $y_2(t) = e^{4t}\begin{pmatrix}1\\1\end{pmatrix} \in S$

\[w(t) = |e^{-2t}\begin{pmatrix}1\\-1\end{pmatrix} e^{4t}\begin{pmatrix}1\\1\end{pmatrix}|\]
\[w(0) = \begin{pmatrix}1&1\\-1&1\end{pmatrix} = 2 \neq 0\]
\[\implies S = <\{y_1,y_2\}> \implies y(t) = c_1e^{-2t}\begin{pmatrix}1\\-1\end{pmatrix} + c_2e^{4t}\begin{pmatrix}1\\1\end{pmatrix}\]
