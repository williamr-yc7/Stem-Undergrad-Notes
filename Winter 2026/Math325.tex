...
\end{theorem}
\begin{proof}%to read
    Denote by $\lambda_1,\ldots,\lambda_n$ the real distinct eigenvalues of $A$ to each eigenvalue $\lambda_i$ corresponds a (real) eigenvector $u_i \in \mathbb{R}^n$ A result from linear algebra implies that $u_1,u_2,\ldots,u_n$ are linearly independent. Let
    \[P = \begin{pmatrix}|&|&|& &|\\u_1&u_2&\ldots&u_n\\|&|&|& &|\end{pmatrix}\]
    be invertible.
    \[APe_i = Au_i = \lambda_i u_i\]
    where $u_i$ is the ith column of P.
    \[\implies AP = P\,diag(\lambda_1,\ldots,\lambda_n) = P \begin{pmatrix}\lambda_1&0&\ldots&0\\0&\lambda_2&\ldots&0\\\vdots&\vdots&\ddots&\vdots\\0&0&\ldots&\lambda_n\end{pmatrix}\]
    \[\implies P^{-1}AP = D\]
\end{proof}
\begin{theorem}
