...
\begin{example}
    ...
    \end{center}
\end{example}
\paragraph{4. Polar Form of Complex Numbers}
...
\begin{definition}[Span]
    ...
    \[\operatorname{span}(v_1,\ldots,v_n) = \{\lambda_1 v_1+\cdots+\lambda_n v_n : \lambda_i \in \mathbb{R}\}.\]
\end{definition}
...
\begin{remark}
    If $A=\{v\}\subset \mathbb{R}^2$ with $v\neq 0$, then $\operatorname{span}(v)$ is a line through the origin.
\end{remark}
\begin{example}
    Let \(A = \begin{pmatrix}1\\1\end{pmatrix}\). Then $\operatorname{span}(A) = \left\{ t\begin{pmatrix}1\\1\end{pmatrix} \mid t \in \mathbb{R} \right\},$ which is a line in \(\mathbb{R}^2\).
    \begin{center}
...
Any vector in \(\operatorname{span}(v_1,v_2)\) has the form \(\lambda_1 v_1+\lambda_2 v_2\). Geometrically, this can be illustrated as the
...
\begin{center}
    ...
    \end{tikzpicture}
\end{center}
Furthermore, if \(v_1,v_2\) are linearly dependent such that $v_1 = \lambda v_2$, where $\lambda \in \mathbb{R}$ and $\lambda \neq 0$, then \(\operatorname{span}(v_1,v_2)\) is a line in \(\mathbb{R}^2\) and $span(v_1) = span(v_2)$. Geometrically, this is a straight line through the origin in the direction of \(v_1\) (and \(v_2\)).
\begin{center}
    \begin{tikzpicture}[scale=1.1, >=Stealth]
        \draw[->] (-3,0) -- (3,0);
        \draw[->] (0,-3) -- (0,3);
...
\paragraph{1. Span in $\mathbb{C}^n$}
...
\begin{example}
    ...
    Find $\operatorname{span}_{\mathbb{C}}(v_1,v_2)$.

    Take $\lambda_1,\lambda_2\in\mathbb{C}$. Then
    ...
    Let $z_1=\lambda_1$ and $z_2=i\lambda_2$. Since $\lambda_2$ is arbitrary in $\mathbb{C}$, $z_2$ can be any complex number. Hence every vector
    \[\begin{pmatrix}z_1\\z_2\end{pmatrix}\in\mathbb{C}^2\]
    can be obtained.
    Therefore, $\operatorname{span}_{\mathbb{C}}(v_1,v_2)=\mathbb{C}^2.$
\end{example}
...
\end{proposition}
To verify that the standard basis really is a basis, two conditions must be satisfied: it spans \(\mathbb{R}^n\) and is linearly independent. To prove the standard basis is a basis, please refer to the Appendix (\ref{subsec:SB}).
...
\begin{example}[Real vs complex span of the same vectors]
    ...
    Over $\mathbb{C}$,
    \[\operatorname{span}_{\mathbb{C}}(e_1,e_2)=\mathbb{C}^2, \qquad \dim_{\mathbb{C}}\mathbb{C}^2=2.\]
    Over $\mathbb{R}$, complex scalars are not allowed, so we need the four real vectors
    \[e_1,\ e_2,\ ie_1,\ ie_2.\]
    These form a basis, hence
    \[\dim_{\mathbb{R}}\mathbb{C}^2=4.\]
    Thus the same space has different dimensions depending on the field.
\end{example}
...
\begin{definition}[Vector space over a field]
    Let $k$ be a field. A vector space over $k$ is a set $V$
    with addition and scalar multiplication such that
    \[\text{(i) } v_1+v_2\in V, \qquad\text{(ii) } \lambda v\in V, \qquad\text{(iii) } 0\in V,\]
    and satisfying the usual vector space axioms
    (see the Appendix (\ref{subsec:VSA}) for the complete list).
\end{definition}
...
\subsubsection{Function Spaces}%to read
...
\begin{definition}[Basis]
    A list $(v_1,\ldots,v_n)$ in a vector space $V$ is a \textbf{basis} if
    \begin{enumerate}
        \item it spans $V$, and
...
Let $(v_1,\ldots,v_n) \in k^m$ and set $U=\operatorname{span}(v_1,\ldots,v_n)$.
\begin{enumerate}
...
\[(v_1,\ldots,v_n)\text{ is a basis} \iff \dim(\operatorname{span}(v_1,\ldots,v_n)) = n.\]
...
\begin{definition}[Direct Sum]
    ...
    \[U+V=\{u+v : u\in U,\, v\in V\}.\]
    We say the sum is \textbf{direct}, written $U \oplus V$, if
    \[U \cap V = \{0\}.\]
\end{definition}
\begin{proposition}
...
Proof by contradiction: Asumption: $(e_1,\ldots,e_n)$ not LI: one vector is in the span of the other $(\lambda_1e_1 + \ldots + \lambda_ne_n = 0)$. $e_k \in span(e_1,e_2,\ldots,e_k,\ldots,e_n)$.
