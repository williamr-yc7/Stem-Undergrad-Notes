...
\begin{example}[Complex numbers as a real vector space]
    ...
    Now consider the space of $n \times n$ complex matrices $M_n(\mathbb{C})$. Each entry $z_{ij} \in \mathbb{C}$ can be written as $z_{ij} = x_{ij} + y_{ij} i$ with $x_{ij},y_{ij} \in \mathbb{R}$. Hence a standard basis over $\mathbb{C}$ consists of the matrices $E_{ij}$ with a 1 in the $(i,j)$-th entry and 0 elsewhere, giving
...
\]
be the space of all polynomials with real coefficients.  
...
Then
\[
\text{span}\{v_1\} = \text{constant polynomials}, \quad
\text{span}\{v_1,v_2\} = P_1,
\]
and in general
\[
...
\begin{definition}[Basis]
    Let \(V\) be a vector space. A set of vectors \(B=\{v_1,\ldots,v_k\}\subset V\) is called a basis of \(V\) if
    \begin{enumerate}
        \item \(B\) spans \(V\), and
