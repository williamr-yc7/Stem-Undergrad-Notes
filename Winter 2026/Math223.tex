...
\end{definition}
\begin{example}
    \[A = \begin{pmatrix}1\\1\end{pmatrix}\in \mathbb{R}^2\]
\end{example}
...
Linear combination: \(v = \lambda_1v_1 + \ldots + \lambda_nv_n\), where lambda is a scalar and v is a vector.
example: \(\lambda_1v_1 + \lambda_2v_2 = 2\begin{pmatrix}1\\3\end{pmatrix} + 4\begin{pmatrix}2\\5\end{pmatrix}\)

Span:
\[Span(v_1,\ldots,v_n) = \{\lambda_2v1+\ldots+\lambda_nv_n : \lambda_i \in \mathbb{R}\}\]
example:
\[span\begin{pmatrix}
    1\\1
\end{pmatrix}\]
draw a graf with a linear function with \(span\begin{pmatrix}1\\1
...
\node[right] at (2.3,2.3) {$\text{Span}\!\begin{pmatrix}1\\1\end{pmatrix}$};
\end{tikzpicture}
...
\node at (3.2,3.2) {$\in \text{Span}(v_1,v_2)$};
\end{tikzpicture}
...
\node at (1.8,-1.6) {$\text{Span}(v_1)=\text{Span}(v_2)$};
\end{tikzpicture}

now in \(\mathbb{C}\):
\(v_1, \ldots, v_n \in \mathbb{C}^n\).
Linear combination: \(v = \lambda_1v_1 + \ldots + \lambda_nv_n\), where lambda is a scalar and v is a vector and \(\lambda_i \in \mathbb{C}\).
example: \(\lambda_1v_1 + \lambda_2v_2 = 2\begin{pmatrix}1\\3\end{pmatrix} + 4\begin{pmatrix}2\\5\end{pmatrix}\)

Span:
\[Span_\mathbb{C}(v_1,\ldots,v_n) = \{\lambda_2v1+\ldots+\lambda_nv_n : \lambda_i \in \mathbb{C}\}\]
example:
\[span\begin{pmatrix}
    2i\\2
...
So:
\[\mathbb{R}^n = \{\begin{pmatrix}x_1\\\ldots\\x_n\end{pmatrix} : x_i \in \mathbb{R}\} \qquad \mathbb{C}^n = \{\begin{pmatrix}x_1\\\ldots\\x_n\end{pmatrix} : x_i \in \mathbb{C}\}\]

Standard basis:
\[\mathbb{R}^n = \{\begin{pmatrix}x_1\\\ldots\\x_n\end{pmatrix} : x_i \in \mathbb{R}\}\]
\[e_1 = \begin{pmatrix}
    1\\0\\\vdots\\0
\end{pmatrix} \ldots e_n = \begin{pmatrix}0\\\vdots\\0\\1\end{pmatrix}\]
Basis -> (span + LI)

Claim: These vectors span \(\mathbb{R}^n\)
Proof: To show an arbitrary vector in \(\mathbb{R}\) can be written as a linear combination of \(e_1,\ldots,e_n\).

\[Let \begin{pmatrix}
    x_1\\\vdots\\x_n
\end{pmatrix} \in \mathbb{R}^n = \begin{pmatrix}
    x_1\\\vdots\\x_n
\end{pmatrix} + \begin{pmatrix} x_2\\\vdots\\x_n\end{pmatrix} + \ldots + \begin{pmatrix}
    0\\\vdots\\x_n
\end{pmatrix} = x_1e_1 + x_2e_2 + \ldots + x_ne_n : Linear combination\]

Example:
\[\mathbb{C}^2 = span_\mathbb{C}(e_1,e_2) = \{z_1e_1 + z_2e_2 : z_1,z_2 \in \mathbb{C}\}\]
...
\[= x_1e_1 + x_2e_2 + y_1(ie_1) + y_2(ie_2)\]
\[= span_\mathbb{R}(\begin{pmatrix}
    1\\0
...
\[\mathbb{C} = span_\mathbb{C}(1) = \{\lambda : \lambda \in \mathbb{C}\}\]
\[\mathbb{C} = span_\mathbb{R}(1,i) = \{x + iy : x, y \in \mathbb{R}\}\]

Abstract vector space:
V = a set
vector space = addition (\(v_1 + v_2\)), scalar multiplication (\(\lambda v\) where \(\lambda\) is a field \(k\)) + axioms

Def: Let \(k\) be a field. A vector space over \(k\) is a set \(V\) and two operations
\[\begin{cases}
    v_1 + v_2 \in V, v_1,v_2 \in V\\
    \lambda v \in V, v \in V, \lambda \in k
\end{cases}
+ 8 axioms.\]
...
There exists a zero \(0 (origin of V) \in V [0 + v = v]\)
\(-v\) makes sense
\(u + v = v + u\)
...
\[K^n = \{\begin{pmatrix}
    x_1\\\vdots\\x_n
\end{pmatrix} : x_i \in K\}\]
is a vector space over K
Remark:
\[span_\mathbb{Z}(e_1,e_2) = \{\lambda_1 e_1 + \lambda_2 e_2 : \lambda_i \in \mathbb{Z}\}\]
