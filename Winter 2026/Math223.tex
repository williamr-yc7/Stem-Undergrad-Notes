...
\medskip
In practice, the key facts to remember are:
\[
\text{(i) } v_1 + v_2 \in V, \quad
\text{(ii) } \lambda v \in V, \quad
\text{(iii) } 0 \in V.
\]
\end{definition}
...
\subsection{Basis}%to organize
\marginpar{January 30, 2026}
\begin{definition}
    $(v_1,\ldots,v_n)$ basis if and only if it spans \(V\) and is linearly independent.
\end{definition}
\begin{example}
    \[e_1 = \begin{pmatrix}1\\0\\0\end{pmatrix}, \quad e_2 = \begin{pmatrix}0\\1\\0\end{pmatrix}, \quad e_3 = \begin{pmatrix}0\\0\\1\end{pmatrix}.\]
    are basis of $\mathbb{R}^3$.
\end{example}
\begin{proof}
    Prove it is a spanning set.
    Span: Let $v = \begin{pmatrix}x\\y\\z\end{pmatrix} \in \mathbb{R}^3$. Prove $v =$ linear combination of $e_1, e_2, e_3$.
...
\end{proof}
\begin{example}
    All standard basis are basis in the same span + LI.
    \begin{itemize}
        \item $\mathbb{R}^n (e_1, \ldots, e_n)$
        \item $M_n(k) e_{ij}$
        \item $P_n(k) 1,x,\ldots,x^{n}$
        \item $F(D)$ D finite $\{\delta_x, x \in D\}$
    \end{itemize}
\end{example}
for $\mathbb{R}^3$, and $v_1,\ldots,v_n \in \mathbb{R}^3$:
If $n < 3$ then $\{v_1,\ldots,v_n\}$ do not span.
If $n > 3$ then $\{v_1,\ldots,v_n\}$ are not LI.
\begin{lemma}
    $V$ vectorspace, $u_1,\ldots,u_n \in V$, $v_1,\ldots,v_m \in V$. 
    If $u_1,\ldots,u_n$ span V and $v_1,\ldots,v_m$ are linearly independent $\implies n \geq m$.
    We can conclude, that a spanning set $\geq$ LI set.
\end{lemma}
Main theorem
\begin{theorem}
    Let \(V\) be a vector space and $v_1,\ldots,v_n \in v$, the following are equivalent:
    Basis: (1) $e = (v_1,\ldots,v_n)$ basis (span+LI)
    Min span: (2) $(v_1,\ldots,v_n)$ span $V$ and $n$ is minimal among all spanning sets.
    Max LI: (3) $(v_1,\ldots,v_n)$ LI in $V$ and $n$ is maximal among all LI sets.
    Coordinates: (4) any $v \in V$ can be written in a unique way. $v = x_1v_1 + \ldots + x_nv_n$, another way to write this is:
    \[[v]_e = \begin{pmatrix}x_1\\\vdots\\x_n\end{pmatrix}.\]
\end{theorem}
Coronary: Any two basis have the same number of vectors (called the dimension of $V$).
\begin{proof}
    Take two basis $(e_1,\ldots,e_n)$ and $(f_1,\ldots,f_n)$ prove $n=m$
\end{proof}
\begin{example}
    Let
    \[v_1 = \begin{pmatrix}
...
\begin{problem}
    ...
    \[\implies v_1,\ldots,v_{n+1} = LI\]
\end{problem}

\marginpar{February 2, 2026}%to organize
\begin{theorem}[Basis]
    TFAE for $(v_1,\ldots,v_n)$ in $V$:
    \begin{enumerate}
        \item Basis = span + LI
        \item Min span = spanning set $(v_1,\ldots,v_n)$, n minimal
        \item Max LI = LI set $(v_1,\ldots,v_n)$, n maximal
        \item Coordinates
    \end{enumerate}    
\end{theorem}
How to find a basis: Let $(v_1,\ldots,v_n)$ BE VECTORS IN $k^n$. Find a basis of $span(v_1,\ldots,v_n)$.
Row space algorithm:
\begin{enumerate}
    \item Write vectors as row in a matrix A.
    \item Gaussian algorithm $\to$ Echele form B.
    \item Rows of B = basis of U
\end{enumerate}
How to prove a set $(v_1,\ldots,v_n)$ of vectors in $K^n$ is a basis? S vectors in $\mathbb{R}^5$
Solution 1: Apply row space alg. Find the dimension of $U = span(v_1,\ldots,v_n)$. $(v_1,\ldots,v_n)$ basis $\iff dimU = n$.
Solution 2: (Apply main basis theorem) Prove span or LI. Prop: $(v_1,\ldots,v_n)$ in $V$ where $dimV = n$ TFAE:
\begin{enumerate}
    \item $(v_1,\ldots,v_n)$ form a basis (span + LI)
    \item $(v_1,\ldots,v_n)$ span
    \item $(v_1,\ldots,v_n)$ LI
\end{enumerate}
Proof of $(1) \iff (2)$:
$(1) \implies (2)$: basis $\implies$ spanning obvious.
$(2) \implies (1)$: Suppose $(v_1,\ldots,v_n)$ is a spanning set, assuming $dim V = n$ Prove $(v_1,\ldots,v_n)$ basis = Prove span + LI. Only need prove LI.
Prof of LI: spanning set of $n = dim V$ vectors $\implies$ this is a minimal ($\implies LI$) spanning set
Theorem (transfer theorem): $\begin{cases}u_1,\ldots,u_n span V\\ v_1,\ldots,v_n LI\end{cases} \implies$ both are Basis
$u_i = X^i$ span $P_n$, $v_i = (x-a)^i$ LI $\implies$ both are basis.
