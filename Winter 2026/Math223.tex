...
\begin{definition}[Vector space over a field]
    Let $k$ be a field. A \textbf{vector space} over $k$ is a set $V$ with addition and scalar multiplication such that
...
\end{proposition}
\begin{proof}[Proof that $U+V = \Span(U \cup V)$]
...
\begin{solution}[Span 2]
    \[u = au_1 + bu_2 + cu_3\]
    \[\begin{pmatrix}2\\10\\7\\0\end{pmatrix} = a\begin{pmatrix}3\\10\\7\\0\end{pmatrix} + b\begin{pmatrix}1\\3\\-2\\0\end{pmatrix} + c\begin{pmatrix}2\\8\\1\\0\end{pmatrix}\]
    Solve the following using gaussian elimination:
    \[\begin{amatrix}{2}3 & 1 & 2 \\  a & b & c\end{amatrix}
\]
\end{solution}
