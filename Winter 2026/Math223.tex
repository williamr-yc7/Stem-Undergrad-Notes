...
\end{example}
\begin{definition}[Matrix Spaces]%to organize
...
\begin{definition}[Function Spaces]
    Let \(D\) be a set and \(k\) a field. The set of all functions from \(D\) to \(k\) is
    \[F(D,k) = \{ f : D \to k \},\]
    which forms a vector space over \(k\) under pointwise addition and scalar multiplication:
    \[(f+g)(x) = f(x) + g(x), \quad (\lambda f)(x) = \lambda f(x), \quad \forall x \in D.\]
    If \(D\) is finite, e.g. \(D = \{1,2,\dots,n\}\), then \(\{ \delta_1, \dots, \delta_n \}\) is a \textbf{standard basis}, where \(\delta_i(j) = 1\) if \(i=j\) and 0 otherwise.  
    If \(D = \mathbb{R}\) and \(k = \mathbb{R}\), we usually consider the functions drawn with horizontal axis \(D\) and vertical axis \(k\). In general, no finite standard basis exists.
\end{definition}
\begin{definition}[Function Spaces]
...
\textbf{Standard basis (finite case):} If $D = \{1, \dots, n\}$, define the Kronecker functions
\[
\delta_i(j) =
\begin{cases}
1, & i=j,\\
...
\end{cases}
\]
Then $\{\delta_1, \dots, \delta_n\}$ is a standard basis of $F(D,k)$, and every $f \in F(D,k)$ can be written uniquely as
\[
f = \sum_{i=1}^n f(i) \delta_i.
\]

\textbf{Infinite case:} If $D$ is infinite (e.g., $D = \mathbb{R}$), no finite standard basis exists.
\end{definition}%to organize
