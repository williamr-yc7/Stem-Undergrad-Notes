...
\begin{definition}[Vector Space]
    Let $k$ be a field. A vector space over $k$ (or K-vectorspace) is a set $V$ with addition and scalar multiplication such that
...
\end{definition}
\begin{remark}
We often use the term abstract vector space to emphasize that the elements of $V$ are not restricted to arrows or tuples in $\mathbb{R}^n$, but can be any objects (like functions or tensors) that satisfy the axioms.
\end{remark}
