...
\end{itemize}
Voltage of a battery = voltage for the entire circuit.
...
\end{itemize}
At $t=0$, capacitors acts like a short circuit. While at $t=\infty$, capacitors acts like an open circuit.
...
RL Charging:
\begin{itemize}
    \item Voltage: $V(t) = V e^{-\frac{t}{\tau}}$
    \item Current: $i(t) = \frac{V}{R}(1-e^{-\frac{t}{\tau}})$
\end{itemize}
At $t=0$, inductors acts like an open circuit, meaning no current goes through the inductor's branch. While at $t=\infty$, inductors acts like a short circuit.

RL Discharging:
\begin{itemize}
    \item Voltage: $V(t) = V_0 e^{-\frac{t}{\tau}}$
    \item Current: $I(t) = I_0 e^{-\frac{t}{\tau}}$
\end{itemize}
...
\textbf{Miscelleneous}
Transient: When you turn a circuit on, this par is the messy start up that dies away (usually $e^{-\frac{t}{\tau}}$).
Steady-state: what the circuit does forever after it settles down.
Filtering rule:
\begin{itemize}
    \item Low Pass: Blocks fast signals (high $f$) and lets slow one through. Usually an RC circuit with output across capacitor. And in an LR its when the signal goes through an inductor first.
    \item High-Pass: Blocks slow/DC signals and lets fast ones through. Usually in an LR circuit where the resistor is put before an inductor or on an RC with output across resistor (capacitor before resistor).
\end{itemize}
Water analogy: Resistor is friction/damping, it sucks energy out. Inductor is the inertia/mass, it hates changing current. Capacitor is like a spring, it stores energy and pushes back.
Phase shifts (ICE vs ELI): In a capacitor, current leads voltage (ICE), and in an inductor, voltage leads current (ELI).
Damping Cases: In an RLC circuit, "Damping" is just how much friction (R) you have compared to the "bounciness" (L or C).
Underdamped: Lots of singing/oscillations before it stops.
Overdamped: No ringing; it just slowly crowls to a stop.
Critically damped: The fastest way to get to 0 without ringing.
Integrator: its a circuit where output $\alpha$ to area under input, when you have $\frac{1}{j \omega}$
Differentiator: output is input's rate of change, found when you have $j\omega$.
Complex impedance $z$: This is the most important. Use it like the way you do for resistance ($V=IZ$).
Decibels (dB) for Bode plots: This is how we measure "Gain" (how much signal gets through).
Gain in dB = $20\log_{10}(\frac{V_{out}}{V_{in}})$
$-3dB$ point: This is the "cutoff" where signal is half-power.
The cutoff frequency $(w_C)$: The "Corner" of the Bode plot.
RC circuit: $w_C = \frac{1}{RC}$
LR circuit: $w_C = \frac{R}{L}$
Tip: If you see a circuit with sine wave input, don't use ODE. Immediately replace $L$ and $C$ with their $z$ formula and solve it like a simple resistor divider.
