...
\begin{definition}[Magnetic Field]
    ...
    where $\mu_0$ is the permeability of free space\footnote{The permeability of free space is $\mu_0 = 4\pi \times 10^{-7}\,\text{T·m/A}$.} and $I$ is the current. The SI unit of magnetic field is the tesla (T).
\end{definition}
...
\end{definition}
In general, an alternating current source is represented in circuit diagrams by a circular source symbol containing a sinusoidal waveform. This indicates that the source provides a time-varying current, typically sinusoidal in form.
\begin{center}
    \begin{figure}[H]
        \centering
...
\begin{definition}[Kirchhoff's Voltage Law]
    Kirchhoff's Voltage Law (KVL) states that the algebraic sum of all potential differences around any closed loop in a circuit is equal to zero.
\end{definition}
...
\begin{definition}[Kirchhoff's Current Law]
    The Kirchhoff's Current Law states that the sum of the currents entering a node is equal to the sum of the currents leaving the node.
\end{definition}
...
\begin{definition}[Transformer]
    A transformer is a device consisting of two coils of wire, called the primary and secondary, magnetically coupled through a shared core. The main purpose of a transformer is to transfer energy between circuits to change voltage levels. A changing current in the primary coil induces a voltage in the secondary coil by mutual inductance. The symbol for a transformer is
    \begin{center}
        \begin{circuitikz}
            \draw
...
If the primary current is sinusoidal, for example $I_1(t) = I_0 \sin(\omega t)$, then
\[V_2(t) = M \frac{dI_1}{dt} = M \omega I_0 \cos(\omega t),\]
so the secondary voltage is also sinusoidal but shifted in phase by $\frac{\pi}{2}$ relative to the current.\\
For an ideal transformer, energy losses are neglected and power is conserved:
\[V_1 I_1 = V_2 I_2.\]
This leads to the transformer relations
\[\frac{V_1}{V_2} = \frac{N_1}{N_2}, \qquad \frac{I_1}{I_2} = \frac{N_2}{N_1},\]
where $\frac{N_1}{N_2}$ is the turns ratio in the two coils.\\
\begin{remark}
    Long-distance power lines use high-voltage AC because it significantly reduces energy loss during transmission over long distances, as higher voltage means lower current, which in turn minimises heat loss within the power lines due to resistance; this is achieved efficiently with transformers that can easily step up and down AC voltage levels as needed.
\end{remark}
...
\end{definition}
Common circuit symbols for inductors are shown below.
...
Energy is stored in the magnetic field within the inductor coil. If the voltage applied to the coil were constant, then the current in the coil would rise. 
\begin{center}
    \begin{tikzpicture}
        \draw[->] (0,0) -- (5,0) node[right] {$t$};
    ...
    \end{tikzpicture}
\end{center}
The energy stored is the amount of work done by the current, which is given by
\[W = \int_{0}^{t} V \cdot idt = \int L\frac{di}{dt}i = \frac{1}{2}LI^2.\]
\subsubsection{Networks of Inductors}
\paragraph{Inductors in Series}
When inductors are connected in series, the same current flows through each element. The total voltage across the combination is the sum of the individual voltages:
\[V_{\text{total}} = \sum_{i=1}^{N} V_i.\]
Since $V_i = L_i \frac{dI}{dt}$ and the current $I$ is the same in each inductor, we obtain
\[V_{\text{total}} = \left( \sum_{i=1}^{N} L_i \right) \frac{dI}{dt}.\]
Thus, the equivalent inductance is
\[L_{\text{total}} = \sum_{i=1}^{N} L_i.\]
\paragraph{Inductors in Parallel}
When inductors are connected in parallel, the voltage across each inductor is the same, while the total current is the sum of the individual currents:
\[I_{\text{total}} = \sum_{i=1}^{N} I_i.\]
Using $V = L_i \frac{dI_i}{dt}$ and the fact that the voltage is common to all branches, we obtain
\[\frac{1}{L_{\text{total}}} = \sum_{i=1}^{N} \frac{1}{L_i}.\]
These relations are formally similar to those of resistors, but they arise from the voltage-current relation of an inductor.
...
\end{definition}
For a solenoid of length $l$, cross-sectional area $A = \pi r^2$, number of turns $N$, and core permeability $\mu$, the inductance is
\[L = \frac{\mu N^2 A}{l}.\]
\begin{figure}[H]
    \centering
...
\begin{definition}[RC Circuit]
    An RC circuit is a circuit consisting of a resistor $R$, a capacitor $C$, and a constant source $V_0$.
\end{definition}
\begin{example}
    An example of an RC circuit is shown in the figure below.
    \begin{center}
        \begin{figure}[H]
            \centering
...
\end{definition}
\begin{example}
    Consider this circuit, at the moment it is turned on (switch connects to 'A'). If the inductor (coil) was replaced with a short, a current $\frac{V}{R}$ would flow immediately, with all of the voltage drop across the resistance $R$.
    \begin{center}
        \begin{figure}[H]
            \label{LR-circuit}
            \centering
        ...
        \end{figure}
    \end{center}
    The current flow produces a magnetic flux through the coil. Lenz's law says that the coil will attempt to produce a field that opposes the change in B-field. To do this, it induces a voltage V (equal to the battery's voltage) across the coil, with the upper end positive and the lower negative. Therefore, just at the instant the switch is closed, the current flow will be zero.
\end{example}
For a single coil,  the voltage-current relationship is
\[V = L \frac{di}{dt},\]
where $L$ is called the self-inductance of the coil, or simply the inductance. $L$ depends on the number of turns of wire, and the magnetic properties of the material within the coil. Any current-carrying conductor even a straight wire is surrounded by a magnetic field, and hence has some self-inductance.

When the switch is connected to terminal A in Figure~\eqref{LR-circuit}, the current in the inductor increases according to
\[I(t) = \frac{V}{R}\left(1 - e^{-\frac{Rt}{L}}\right),\]
while the voltage across the inductor decreases as
\[V_L(t) = V e^{-\frac{Rt}{L}}.\]
The characteristic time constant of the circuit is
\[\tau = \frac{L}{R}.\]

\section{RC and LR Circuits with AC Driving}
\marginpar{January 29, 2026}
\subsection{Driving Frequency and Time Constant}
For a periodic input signal with period $T$ and angular frequency $\omega$,
\[\omega = \frac{2\pi}{T} = 2\pi \nu.\]
The characteristic time constants are:
\[\tau_{RC} = RC\]
\[\tau_{LR} = \frac{L}{R}\]
The circuit behaviour depends on the comparison between $T$ and $\tau$:
\begin{itemize}
    \item Low frequency: $T \gg \tau$
    \item High frequency: $T \ll \tau$
\end{itemize}
The key idea is that the response depends on the ratio between the driving period and the circuit time constant.

\subsection{RC Circuit with Square-Wave Input}
\subsubsection*{Low Frequency ($T \gg \tau$)}
\begin{itemize}
    \item Capacitor fully charges and discharges.
    \item Current similar to pure resistor case.
    \item Very little voltage remains across the capacitor.
...
\begin{itemize}
    ...
    \item Voltage dropped mostly across the capacitor.
    \item Current appears as sharp spikes at switching.
    \item Capacitor behaves approximately like an open circuit.
\end{itemize}

\subsection{Integrator and Differentiator Behaviour (RC)}
For slowly varying signals (low frequency),
\[V_C(t) \approx \frac{1}{RC} \int V_{in}(t)\, dt\]
The capacitor acts as an integrator. A square wave input produces a triangle-like voltage output. For rapidly varying signals (high frequency),
\[V_R(t) \approx RC \frac{dV_{in}}{dt}\]
The resistor behaves as a \textbf{differentiator}.  
Sharp transitions in the input produce current spikes.

\subsection{LR Circuit with AC Input}
The LR circuit is mathematically analogous to the RC circuit.
\[\tau_{LR} = \frac{L}{R}\]
\begin{itemize}
...
\begin{center}
    ...
    \hline
    RC & Capacitor $\approx$ short & Capacitor $\approx$ open \\
    \hline
...
\begin{itemize}
    \item A capacitor blocks DC.
    \item An inductor passes DC (after long time).
    \item A capacitor resists changes in voltage.
    \item An inductor resists changes in current.
\end{itemize}
...
\begin{itemize}
    \item Ohm's Law: \eqref{Ohmslawformula}
...
\begin{itemize}
    ...
    Parallel: $R_{eq} = \frac{1}{\frac{1}{R_1}+\frac{1}{R_2}+\cdots+\frac{1}{R_n}}$

    \item Capacitors  
    Series: $C_{eq} = \frac{1}{\frac{1}{C_1}+\frac{1}{C_2}+\cdots+\frac{1}{C_n}}$  
    Parallel: $C_{eq} = C_1 + C_2 + \cdots + C_n$

    \item Inductors  
    Series: $L_{eq} = L_1 + L_2 + \cdots + L_n$  
    Parallel: $L_{eq} = \frac{1}{\frac{1}{L_1}+\frac{1}{L_2}+\cdots+\frac{1}{L_n}}$
\end{itemize}

\textbf{Capacitors}
\begin{itemize}
    \item Charge relation: $Q = CV$
    \item Capacitance: $C = \frac{\epsilon A}{d}$
    \item Permittivity of free space: $\epsilon_0 = 8.85 \times 10^{-12}$
\end{itemize}
...
Charging:
\begin{itemize}
    \item Voltage: $V_c(t) = V(1-e^{-\frac{t}{\tau}})$
    \item Current: $i(t) = \frac{V}{R}e^{-\frac{t}{\tau}}$
    \item Charge: $Q(t) = CV(1-e^{-\frac{t}{\tau}})$
\end{itemize}
At $t=0$, the capacitor behaves like a short circuit. As $t \rightarrow \infty$, it behaves like an open circuit.

Discharging:
\begin{itemize}
    \item Voltage: $V_c(t) = V_0 e^{-\frac{t}{\tau}}$
    \item Current: $I(t) = I_0 e^{-\frac{t}{\tau}}$
    \item Charge: $Q(t) = Q_0 e^{-\frac{t}{\tau}}$
\end{itemize}

Stored energy:
\begin{itemize}
    \item Capacitor: $W = \frac{1}{2}CV^2$
\end{itemize}

\textbf{Inductors}
\begin{itemize}
    \item Inductor voltage: $V = L\frac{dI}{dt}$
    \item Inductance: $L = \frac{\mu N A}{l}$
    \item Permeability of free space: $\mu_0 = 4\pi \times 10^{-7}$
\end{itemize}

RL Charging:
\begin{itemize}
    \item Voltage: $V_L(t) = V e^{-\frac{t}{\tau}}$
    \item Current: $i(t) = \frac{V}{R}(1-e^{-\frac{t}{\tau}})$
\end{itemize}
At $t=0$, an inductor behaves like an open circuit. As $t \rightarrow \infty$, it behaves like a short circuit.

RL Discharging:
\begin{itemize}
    \item Voltage: $V_L(t) = V_0 e^{-\frac{t}{\tau}}$
    \item Current: $I(t) = I_0 e^{-\frac{t}{\tau}}$
\end{itemize}

Stored energy:
\begin{itemize}
    \item Inductor: $W = \frac{1}{2}LI^2$
\end{itemize}

\textbf{Energy Dissipation}
\begin{itemize}
    \item Resistor: $U = \int RI^2(t)\,dt$
\end{itemize}

\textbf{RLC Circuits}
\begin{itemize}
    \item Natural frequency: $\omega_0 = \frac{1}{\sqrt{LC}}$
    \item Damping factor: $\Gamma = \frac{R}{L}$
\end{itemize}
...
\begin{itemize}
    \item Resistor: $Z_R = R$
    \item Capacitor: $Z_C = \frac{1}{j\omega C}$
    \item Inductor: $Z_L = j\omega L$
\end{itemize}

\textbf{Kirchhoff's Law}
\begin{itemize}
    \item Kirchhoff's Law: \eqref{klawformula}
\end{itemize}

\textbf{Miscellaneous}

Transient: The short initial response when a circuit is switched on, typically involving terms like $e^{-\frac{t}{\tau}}$.

Steady-state: The long-term behavior of the circuit after transients vanish.

Filtering rule:
\begin{itemize}
    \item Low-pass filter: Blocks high-frequency signals and allows low-frequency signals to pass. Often an RC circuit with output across the capacitor or an LR circuit where the inductor appears first in the signal path.
    \item High-pass filter: Blocks low-frequency or DC signals and allows high-frequency signals to pass. Commonly an RC circuit with output across the resistor or an LR circuit where the resistor precedes the inductor.
\end{itemize}

Water analogy:  
Resistor behaves like friction and dissipates energy.  
Inductor behaves like inertia and resists changes in current.  
Capacitor behaves like a spring and stores electrical energy.

Phase shifts (ICE vs ELI):  
In a capacitor, current leads voltage (ICE).  
In an inductor, voltage leads current (ELI).

Damping cases in an RLC circuit:
\begin{itemize}
    \item Underdamped: Oscillatory behavior before settling.
    \item Overdamped: No oscillations; the system slowly approaches equilibrium.
    \item Critically damped: The fastest return to equilibrium without oscillations.
\end{itemize}

Integrator: Circuit where the output is proportional to the integral of the input, associated with $\frac{1}{j\omega}$.

Differentiator: Circuit where the output corresponds to the rate of change of the input, associated with $j\omega$.

Complex impedance $Z$: Treat impedance like resistance when solving AC circuits, using $V = IZ$.

Decibels (dB) for Bode plots: Used to measure gain.
\[
...
\begin{itemize}
    \item RC circuit: $\omega_c = \frac{1}{RC}$
    \item LR circuit: $\omega_c = \frac{R}{L}$
\end{itemize}

Tip: For sinusoidal inputs, replace $L$ and $C$ with their impedance expressions and analyze the circuit using standard algebra instead of solving differential equations.

\section{Useful Links}
\noindent\textbf{Inductors (visual explanation):}https://www.youtube.com/shorts/lXk4CQDbr4Q\\
\textbf{Capacitors (visual explanation):}https://www.youtube.com/shorts/PYrTLQctdwg\\
\textbf{Resistors (introduction):}https://www.youtube.com/watch?v=5HI33hSZoH4\\
