...
\thispagestyle{empty}
\begin{abstract}
    These notes summarize the main topics of MATH 248: Honours Vector Calculus, taught by Prof. Pengfei Guan. The course covers partial derivatives and differentiability in several variables, the Jacobian, extrema and the Hessian, scalar and vector fields, orthogonal curvilinear coordinates, multiple integrals, arc length, surface area, and volume. It also includes line and surface integrals, irrotational and solenoidal fields, Green's theorem, the Divergence theorem, and Stokes' theorem.
\end{abstract}
\noindent\rule{\textwidth}{0.4pt}
\clearpage
...
\begin{theorem}[The Power Rule]
    Let \(n\) be a any number. If \(f(x) = x^n\), $\forall n \in \mathbb{Q}$\textup{[\footnote{$\mathbb{Q}$ is the set of all rational numbers.}]}, then
    \[\frac{d}{dx}f(x) = f'(x) = nx^{n - 1}.\]
\end{theorem}
\begin{theorem}[The Chain Rule]
    Let \(f\) and \(g\) be functions from \(\mathbb{R} \to \mathbb{R}\). \(\forall x \in \mathrm{dom}(g)\)\textup{[\footnote{$\mathrm{dom}$ means `domain'.}]} for which \(g\) is \(C^1\) at \(x\) and \(f\) is \(C^1\) at \(g(x)\), the derivative of the composite function \(
...
There are more ways to integrate a function, such as trig substitution and ..., which are not used in this course as they take longer time to solve and is not the primary focus of the class
...
\begin{problem}[Triangle Inequality]
    Prove that 
    \[||x - y||||x + y|| \leq ||x||^2 + ||y||^2, \forall x,y \in \mathbb{R}^n.\]
\end{problem}

\section{Differentiability}
For this topic, the class of fall 2025 did not use the epsilon-delta definition of differentiability.
...
\end{theorem}
\begin{corollary}[Jacobian Matrix]
...
\end{enumerate}
\begin{lemma}[Path Analysis]
    %reword
    Path Analysis is the process in which you apply different random values to a variable.
\end{lemma}
...
\begin{theorem}[First-Order Taylor Formula]
    ...
    where the remainder \(R_1(\mathbf{x_0}, \mathbf{h})/||\mathbf{h}||\) \(\rightarrow\) 0 as \(\mathbf{h} \to 0\) in \(\mathbb{R}^n\).
\end{theorem}
...
\section{Answer key}
\begin{solution}[AM-GM Inequality]
...
\end{solution}
\begin{solution}[AM-GM Inequality 2]
...
\begin{solution}[Cauchy-Schwarz Inequality]
...
\begin{solution}[Triangle Inequality]
...
\begin{solution}[Differentiability]
...
\end{solution}
\begin{solution}[Differentiability 2]
    First we check for continuity at (0,0) by using path analysis:
